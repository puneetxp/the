...
class Auth {
   ...
   public static function register() {
      ...
      if (User::find($user['email'], 'email')?->array() == null) {
         ...
         if (is_array($auth)) {
            session_destroy();
            (Req::one('remember_me')) ? session_set_cookie_params(lifetime_or_options: 0, path: '/', domain: web, secure: true, httponly: true) : session_set_cookie_params(lifetime_or_options: 1440, path: '/', domain: web, secure: true, httponly: true);
            session_start();
            $_SESSION['user_id'] = $auth['id'];
            $auth['roles'] = Sessions::roles();
            return Response::json(array_intersect_key($auth, array_flip(["name", "email", "id", "roles"])));
