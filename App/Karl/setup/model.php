...
function model($table) {
   ...
   $fillable .= "']";
   $controllers = count($relations_key) > 0 ? 'use App\Karl\Model\{ ' . implode(',', array_map('ucfirst', $relations_key)) . ' };' : '';
   return php_w(
           'namespace App\Karl\Model;' .
           'use App\Karl\Model;' . $controllers . class_wrapper(ucfirst($table['name'] . ' extends Model'),
